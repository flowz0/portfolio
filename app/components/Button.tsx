...
export default function Button({ text, className, submit = false, variant = 'primary' }: ButtonProps) {
    ...
    return (
        <button
            ...
            className={`${baseClass} ${variantClasses[variant]} ${className} ${noto_sans.className} uppercase font-semibold tracking-wide transition ease-in-out delay-75 duration-300 hover:scale-105 hover:-translate-y-1 active:scale-95`}
        >
            {text}
        </button>
    )
